...
# ============================================================
# 비밀번호 잠금 (URL 토큰으로 인증 유지)
# ============================================================
def check_password():
    """비밀번호 입력 화면. URL 토큰으로 자동 재인증."""
    
    # 비밀번호의 sha256 해시 일부를 URL 토큰으로 사용
    AUTH_TOKEN = hashlib.sha256(st.secrets["APP_PASSWORD"].encode()).hexdigest()[:16]
    ...
    def password_entered():
        if st.session_state["password_input"] == st.secrets["APP_PASSWORD"]:
            st.session_state["password_correct"] = True
            # URL에 인증 토큰 추가 (즐겨찾기/새로고침 후 자동 인증)
...
def is_shipping_item(text):
    ...
    shipping_keywords = [
        ...
        '화물비', '화물',
        '차비', '차량비', '운반비', '운반',
        '퀵서비스',  # '퀵' 단독 제거 (퀵출고/퀵픽업 같은 거래방식 표시 보호)
        '용달', '용달비',
...
def clean_raw_text(text):
    ...
    if not text:
        return ""
    s = str(text)
    # 0-1. 거래방식 메타 정보 제거: "퀵출고(현불)", "픽출고(현불)", "현불", "당일출고" 등
    s = re.sub(r'(퀵출고|픽출고|당일출고|현장출고|즉시출고)\s*\([^)]*\)', '', s)
    s = re.sub(r'(퀵출고|픽출고|당일출고|현장출고|즉시출고)', '', s)
    # 0-2. 시작 부분 한쪽 괄호 약자 제거: "DN)실링팬" → "실링팬" (prefix는 코드가 자동으로 붙임)
    s = re.sub(r'^\s*[A-Z]{1,5}\)\s*', '', s)
    # 0-3. 중간 위치 한쪽 괄호 약자 뒤 띄어쓰기 (희귀 케이스 대비): "...XX)단어" → "...XX) 단어"
    s = re.sub(r'([A-Z]{1,5})\)([가-힣A-Za-z])', r'\1) \2', s)
    # 0-4. 인치 약자 한글 변환: 42" → 42인치 (다양한 따옴표 변형 모두 처리)
    s = re.sub(r'(\d+)\s*[\"″”“]', r'\1인치', s)
...
def extract_features(raw_text, memo_text=""):
    ...
    # AS 보증 기간 (1년 아닌 특수한 경우만 규격에 표시. 1년은 일반이라 미표시)
    as_match = re.search(r'AS\s*(\d+)\s*년', full_text, re.IGNORECASE)
    if as_match and int(as_match.group(1)) != 1:
        features['as_warranty'] = f"AS {as_match.group(1)}년"
...
def extract_product_name(raw_text, features, category):
    ...
    # 인치는 매입등에서만 prefix로 빼냄 (다른 카테고리는 사이즈 정보로 품목명에 보존)
    if features['inch'] and category == '매입등':
    ...
    # 6-1. AS 보증 기간 패턴 제거 (1년/2년 무관, 주변 슬래시/플러스 포함)
    # "/AS 2년", "+AS 2년", "AS 2년" 등 모두 처리. features['as_warranty']는 별도로 규격에 추가됨.
    s = re.sub(r'\s*[/+]\s*AS\s*\d+\s*년', '', s, flags=re.IGNORECASE)
    s = re.sub(r'\s*AS\s*\d+\s*년\s*[/+]?', '', s, flags=re.IGNORECASE)
    
    # 6-2. 동일 색상 중복 패턴 잔재 제거 ("화이트+화이트" 같은 경우 색상 제거 후 "+" 잔재)
    # 텍스트 중간/끝의 "+/", "/+", 끝의 "+" 정리
    s = re.sub(r'\+/', '/', s)
    s = re.sub(r'/\+', '/', s)
    s = re.sub(r'\s*\+\s*$', '', s)
...
def build_specification(features, category=None):
    """규격 컬럼: 색상/W/불빛색/N호/AS특수 (모델코드/인치 절대 포함 안 함).
    인치는 매입등이든 실링팬이든 품목명에 들어감 (사이즈 정보)."""
    parts = []
    if features['color']:
    ...
    # AS 보증 (1년이 아닌 특수한 경우만 - 마지막 위치)
    if features.get('as_warranty'):
        parts.append(features['as_warranty'])
    return '/'.join(parts)
...
uploaded_files = st.file_uploader("", type=['jpg', 'jpeg', 'png'], accept_multiple_files=True, key=st.session_state['uploader_key'])

# 새 파일 업로드 시 분석 관련 세션 초기화
if uploaded_files:
...
if uploaded_files:
    # 업로드된 모든 이미지 미리보기 (streamlit 기본 - 확대 아이콘 자동)
    if len(uploaded_files) == 1:
        image = Image.open(uploaded_files[0])
        st.image(image, caption=f'{uploaded_files[0].name}', use_container_width=True)
    else:
        st.markdown(f"### 업로드된 파일 {len(uploaded_files)}장")
...
"""
            ...
            for idx, uploaded_file in enumerate(uploaded_files):
                progress_text.markdown(f"**[{idx+1}/{len(uploaded_files)}]** `{uploaded_file.name}` 분석 중...")
                ...
                try:
                    ...
                    debug_entry = {
                        ...
                        'ai_response_raw': response.text[:3000],  # 처음 3000자만
                    }
                    ...
                    debug_entry['supplier_raw'] = supplier_raw
                    debug_entry['items_count'] = len(items_raw)
                    debug_entry['items_raw'] = items_raw
                    ...
                    for item in items_filtered:
                        ...
                        spec = build_specification(features, category)
            ...
            progress_text.markdown(f"**✅ 분석 완료** ({len(uploaded_files)}장 처리)")
            ...
            if not all_rows:
                ...
                with st.expander("🔍 디버그 정보 (개발자용)", expanded=True):
                    st.write("**AI가 받은 응답을 보여드립니다. 이게 비어있다면 AI가 추출 못한 것이고, 데이터가 있는데도 0개라면 후처리에서 빠진 것입니다.**")
                    for d in debug_info:
                        st.markdown(f"### 📄 {d['filename']}")
                        st.write(f"- supplier 인식: `{d['supplier_raw']}`")
                        st.write(f"- 추출된 행 수: **{d['items_count']}개**")
                        if d.get('items_raw'):
                            st.json(d['items_raw'])
                        st.text_area("AI 원본 응답 (앞 3000자)", d['ai_response_raw'], height=200, key=f"debug_{d['filename']}")
    ...
    if 'df_analyzed' in st.session_state:
        ...
        # ⚠️ 버그 수정: data_editor에서 행 추가/삭제한 결과를 정확히 반영하기 위해
        # df의 행 인덱스를 edited_display에 맞춰 재구성한다.
        # 이전 버그: 컬럼만 덮어쓰면 행 삭제 시 빈 행이 남음 → 코드만 있고 데이터 없는 상태 발생
        
        # display_cols 외의 컬럼(품목코드, 분류 등)은 빈값으로 채움 (코드는 어차피 다음 단계에서 부여)
        non_display_cols = [c for c in df.columns if c not in display_cols]
        
        # edited_display 기반으로 새 df 생성
        new_df = edited_display.copy()
        for col in non_display_cols:
            new_df[col] = ""  # 비-표시 컬럼은 비움 (품목코드 등은 다음 단계에서 채워짐)
        
        # 컬럼 순서 보존 (원본 df와 동일하게)
        new_df = new_df[df.columns.tolist()]
        
        # 빈 행 자동 제거: 품목명이 비어있거나 공백뿐인 행 제외
        new_df = new_df[new_df['품목명'].notna() & (new_df['품목명'].astype(str).str.strip() != '')].reset_index(drop=True)
